...
- 제네릭을 사용하는 이유
  ...
  - Any를 사용하면 매개변수의 타입이 Any인 경우에만 값을 변경할 수 있음. String 타입의 변수 2개를 Any 타입의 변수에 값 복사 하여 값을 교환하기 때문에 실제 변경하려는 String 변수의 값은 변하지 않는다.
...
- 실제 타입 이름을 써주는 대신, 플레이스홀더 T를 사용함. `Dictionary<Key, Value>`, `Array<Element>` 와 같이 의미있는 이름 사용 가능
